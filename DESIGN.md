...
description: A photography-first interface that turns marketing into a museum gallery. Edge-to-edge product tiles alternate light and dark canvases, framed by SF Pro Display headlines with negative letter-spacing and a single accent (#DD7694) interactive color. UI chrome recedes so the product can speak — no decorative gradients, no shadows on chrome, only the one signature drop-shadow under product imagery resting on a surface.

colors:
  primary: "#DD7694"
  primary-focus: "#E48BA5"
  primary-on-dark: "#F0A0B8"
  ink: "#1d1d1f"
  body: "#1d1d1f"
  body-on-dark: "#ffffff"
  body-muted: "#cccccc"
...
- Single accent (`{colors.primary}` — #DD7694) carries every interactive element. No second brand color exists.
...
### Brand & Accent
- **Accent Pink** (`{colors.primary}` — #DD7694): The single brand-level interactive color. All text links, all pill CTAs ("Learn more", "Buy"), and the focus ring root. This is the quiet but universal "click me" signal. Press state shifts to a slightly darker variant via the active scale transform rather than a hex change.
- **Focus Pink** (`{colors.primary-focus}` — #E48BA5): A marginally brighter sibling, reserved for the keyboard focus ring on buttons (`outline: 2px solid`).
- **Light Pink** (`{colors.primary-on-dark}` — #F0A0B8): A brighter variant used on dark surfaces for in-copy links and inline callouts, where the base accent would disappear against the tile background.
...
**`button-primary`** — The signature action. Background `{colors.primary}` (Accent Pink #DD7694), text `{colors.on-primary}` in `{typography.body}` (SF Pro Text 17px / 400), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. The full-pill radius IS the brand action signal.
...
**`text-link-on-dark`** — Inline body links on dark tiles in `{colors.primary-on-dark}` (Light Pink #F0A0B8) — the base accent would disappear against `{colors.surface-tile-1}`.
...
### Do
- Use `{colors.primary}` (Accent Pink #DD7694) for every interactive element — links, pill CTAs, focus signals — and nothing else. The single accent is non-negotiable.
